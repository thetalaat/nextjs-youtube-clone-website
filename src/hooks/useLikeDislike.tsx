        ...
        if (action === "like") {
          switch (likeDislikeStatus) {
            case LikeDislikeStatus.Liked:
              await axios.delete(`/api/video/${videoId}/like`);
              break;
            case LikeDislikeStatus.Disliked:
              await axios.delete(`/api/video/${videoId}/dislike`).then(() => axios.post(`/api/video/${videoId}/like`));
              break;
            default:
              await axios.post(`/api/video/${videoId}/like`);
              break;
          }
        } else if (action === "dislike") {
          switch (likeDislikeStatus) {
            case LikeDislikeStatus.Liked:
              await axios.delete(`/api/video/${videoId}/like`).then(() => axios.post(`/api/video/${videoId}/dislike`));
              break;
            case LikeDislikeStatus.Disliked:
              await axios.delete(`/api/video/${videoId}/dislike`);
              break;
            default:
              await axios.post(`/api/video/${videoId}/dislike`);
              break;
          }
        }
        router.refresh();
