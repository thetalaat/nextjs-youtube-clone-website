...
const VideoDetailsCard: React.FC<VideoDetailsCardProps> = ({ video }) => {
  ...
  const handleDeleteVideo = useCallback(() => {
    if (confirm("Are you sure you want to delete this video?")) {
      axios.delete(video.videoSrc);
      axios.delete(video.thumbnailSrc);
      axios
  ...
  }, [video.id, video.videoSrc, video.thumbnailSrc]);

  return (
    <div key={video.id} className="flex gap-6 justify-between items-center bg-neutral-800 p-4 rounded-lg">
      <Link href={`video/${video.id}`}>
        <Image className="aspect-video object-cover" alt={`${video.title} thumbnail`} src={video.thumbnailSrc} height={90} width={160} />
      </Link>

      <div className="w-2/5 ">
        <h3>{video.title}</h3>
        <p className="test-sm text-neutral-400 line-clamp-2">{video.description}</p>
      </div>
