...
const AnalyticsSummary: React.FC<AnalyticsSummaryProps> = ({ videos }) => {
  ...
  const viewsCount = useMemo(() => videos?.reduce((totalViews, video) => totalViews + video.viewCount, 0), [videos]);

  return (
    <div className="mx-auto flex items-center gap-4">
      <Avatar size={AvatarSize.large} imageSrc={currentChannel?.imageSrc} className="hidden md:inline" />
