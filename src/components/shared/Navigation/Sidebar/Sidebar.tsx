...
const Sidebar: React.FC<SidebarProps> = ({ subscribedChannels }) => {
  ...
  return (
    <>
      {sidebar?.isOpen && <div className="bg-black bg-opacity-50 h-screen w-screen fixed z-30" onClick={sidebar.onClose} />}
      <div className={`fixed w-64 bg-stone-950 z-40 mt-2 px-6 flex flex-col h-screen overflow-scroll no-scrollbar ${sidebar?.isOpen ? "translate-x-0" : "-translate-x-full"} ease-in-out duration-300`}>
        <NavigationHeader />
            ...
            {subscribedChannels.map((subscribedChannel) => {
              return (
                <MenuItem
                  ...
                  logo={<Avatar imageSrc={subscribedChannel.imageSrc} size={AvatarSize.small} className="mr-4" />}
                  round
                  onClick={() => {
                    handleItemClick(() => {
