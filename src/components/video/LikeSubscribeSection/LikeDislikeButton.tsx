...
const LikeDislikeButton: React.FC<LikeDislikeButtonProps> = ({ video }) => {
  ...
  return (
    <div className="flex items-center gap-1 bg-neutral-800 rounded-full px-3 py-2 text-white font-medium">
      <button className="pr-3 border-r-2 border-neutral-600 flex items-center gap-3" onClick={() => toggleLikeDislike("like")}>
        {likeDislikeStatus === LikeDislikeStatus.Liked ? <MdThumbUp className="h-6 w-6" /> : <MdOutlineThumbUp className="h-6 w-6" />}
        <p>{compactNumberFormat(video.likeCount)}</p>
      </button>
      <button className="pl-2" onClick={() => toggleLikeDislike("dislike")}>
        {likeDislikeStatus === LikeDislikeStatus.Disliked ? <MdThumbDown className="h-6 w-6" /> : <MdOutlineThumbDown className="h-6 w-6" />}
      </button>
    </div>
  );
};
